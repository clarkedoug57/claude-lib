...
- `utilities/statement-parsers` — six deterministic parsers, one per layout, each registered by fingerprint and each returning a canonical Statement that has already validated and reconciled to the cent: Simplii chequing, Simplii personal line of credit, Simplii Cash Back Visa, PC World Elite Mastercard, TD line of credit, TD Cash Back Visa. A parser takes the extractor's rows per page — the same rows a staged fixture stores — and never guesses: an unplaceable dated amount row, a missing header or a balance that does not chain is a `ParseError` naming the page and row. The Simplii ledgers take an item's sign from the running-balance delta and refuse a printed amount that disagrees with it (R-593).
...
`fixtures/<issuer>/<document-type>.json` are staged by a human decision — through an app's Statement lab, or from a parser's own output proven against the lab's earlier reading of the same document — redacted by construction, and re-proven on every `npm test`: each validates against the model, balances per section through the gate, passes the verifier against its own raw rows, and carries no redaction residual. A fixture that names a parser in its provenance is that parser's specification: the parser must reproduce it byte for byte from the fixture's own raw rows, and every other parser must refuse them. A fixture that names none is the specification for a parser not yet written. `fixtures/verdicts/` holds the documents the agent must keep refusing.
...
Tags are the versions apps pin (`v0.1.0`, `v0.2.0`, `v0.3.0`, `v0.3.1`, `v0.4.0`). A change to any package bumps the root version; the consuming app's pin is a deliberate reconciliation, never a silent bump.
